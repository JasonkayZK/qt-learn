...
// You may need to build the project (run Qt uic code generator) to get "ui_main_game_window.h" resolved
...
void MainGameWindow::paintEvent(QPaintEvent *event) {
  ...
  QPixmap bmpBlocks("../resources/blocks.bmp");
  QPixmap bmpFaces("../resources/faces.bmp");
  QPixmap bmpFrame("../resources/frame.bmp");
  QPixmap bmpNumber("../resources/timenumber.bmp");

  // 绘制笑脸
  switch (game->gameState) {
    case OVER:
      painter.drawPixmap((game->mCol * blockSize + offsetX * 2) / 2 - 12, spaceY / 2, bmpFaces, 0 * 24, 0, 24,
                         24); // 24是笑脸的边长,锚点在左上，因为工具栏占了些，所以看起来不再中间
      break;
    case PLAYING:
      painter.drawPixmap((game->mCol * blockSize + offsetX * 2) / 2 - 12, spaceY / 2, bmpFaces, 1 * 24, 0, 24,
                         24);
      break;
    case WIN:
      painter.drawPixmap((game->mCol * blockSize + offsetX * 2) / 2 - 12, spaceY / 2, bmpFaces, 2 * 24, 0, 24,
                         24);
      break;
    default:
      painter.drawPixmap((game->mCol * blockSize + offsetX * 2) / 2 - 12, spaceY / 2, bmpFaces, 1 * 24, 0, 24,
  ...
  for (int i = 0; i < game->mRow; i++) {
    for (int j = 0; j < game->mCol; j++) {
      switch (game->gameMap[i][j].curState) {
        ...
        case UN_DIG:
          painter.drawPixmap(j * blockSize + offsetX, i * blockSize + offsetY + spaceY, bmpBlocks,
                             blockSize * 10, 0, blockSize, blockSize);
          break;
        case DUG:
          painter.drawPixmap(j * blockSize + offsetX, i * blockSize + offsetY + spaceY, bmpBlocks,
                             blockSize * game->gameMap[i][j].valueFlag, 0, blockSize, blockSize);
          break;
        case MARKED:
          painter.drawPixmap(j * blockSize + offsetX, i * blockSize + offsetY + spaceY, bmpBlocks,
                             blockSize * 11, 0, blockSize, blockSize);
          break;
        case BOMB:
          painter.drawPixmap(j * blockSize + offsetX, i * blockSize + offsetY + spaceY, bmpBlocks,
                             blockSize * 9, 0, blockSize, blockSize);
          break;
        case WRONG_BOMB:
          if (game->gameState == PLAYING || game->gameState == FAULT) {
            ...
            // 如果游戏已经结束，就显示标错了
            painter.drawPixmap(j * blockSize + offsetX, i * blockSize + offsetY + spaceY, bmpBlocks,
                               blockSize * 12, 0, blockSize, blockSize);
          }
          break;
        default:break;
      }
...
void MainGameWindow::handleGameState(GameModel *game_model) {
  if (game_model->gameState == OVER) {
    timer->stop();
    qDebug() << "you lose!";
  } else if (game_model->gameState == WIN) {
    timer->stop();
    qDebug() << "you win!";
//        // 可以选择弹窗告知游戏胜利成绩
//        QMessageBox::information(this, "win!", "your time: " + QString::number(game_model->timerSeconds) + " s");
//        game_model->gameState = PLAYING; // 赶紧要改回正常状态，不然那个弹窗消失不了
  }
}

void MainGameWindow::mousePressEvent(QMouseEvent *event) {
  if (event->pos().y() < spaceY + offsetY) {
    int x = event->pos().x();
    int y = event->pos().y();
    // 此时判断是否点击笑脸
    ...
    // 此时判断点击的是哪个方块
    // 获得点击坐标
    int px = event->pos().x() - offsetX;
    int py = event->pos().y() - offsetY - spaceY;
    // 换算成方格索引
    int row = py / blockSize;
    int col = px / blockSize;
    // 根据不同情况响应
    switch (event->button()) {
      case Qt::LeftButton:game->digMine(row, col);
        update(); // 每次点击都要重绘
        break;
      case Qt::RightButton:game->markMine(row, col);
        update();
        break;
      default:break;
    }
...
void MainGameWindow::onLevelChooseClicked() {
  auto *actionSender = (QAction *) dynamic_cast<QAction *>(sender());
